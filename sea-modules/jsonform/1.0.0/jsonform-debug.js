 ...
 */
        ...
        (function(serverside, global, $, _, JSON) {
            if (serverside) {
                _ = require("#/underscore/1.4.3/underscore-debug");
            }
            /**
 ...
 */
            var initializeTabs = function(tabs) {
                ...
                var optionSelected = function(e) {
                    var $option = $("option:selected", $(this)), $select = $(this), targetIdx = $option.attr("data-idx") || $option.attr("value"), $target;
                    e.preventDefault();
                ...
                tabs.each(function() {
                    ...
                    $(this).find("select.nav").each(function() {
                        ...
                        var targetIdx = $(this).find("option:selected").attr("data-idx") || $(this).find("option:selected").attr("value");
            ...
            jsonform.elementTypes = {
                    ...
                    onInsert: function(evt, node) {
                        $(node.el).on("click", ".dropdown-menu a", function(evt) {
                            ...
                            if (value) {
                                ...
                                $(node.el).find('a[data-toggle="dropdown"]').addClass(elt.imageButtonClass).attr("style", "max-width:" + width + "px;max-height:" + height + "px").html('<img src="' + (!value.match(/^https?:/) ? prefix : "") + value + suffix + '" alt="" />');
                            } else {
                                $(node.el).find("input").attr("value", "");
                                $(node.el).find('a[data-toggle="dropdown"]').removeClass(elt.imageButtonClass).removeAttr("style").html(elt.imageSelectorTitle || "Select...");
                            }
                        });
                    }
                },
                ...
                checkboxes: {
                    ...
                    onBeforeRender: function(data, node) {
                        ...
                        var template = '<label class="checkbox">' + '<input type="checkbox" <% if (value) { %> checked="checked" <% } %> name="<%= name %>" value="1"<%= (node.disabled? " disabled" : "")%>><span><%= title %></span></label>';
                ...
                array: {
                    ...
                    onInsert: function(evt, node) {
                        ...
                        if (boundaries.minItems >= 0 && curItems < boundaries.minItems) {
                            for (var i = 0; i < boundaries.minItems - 1 && $("> ul > li", $nodeid).length < boundaries.minItems; i++) {
                ...
                tabarray: {
                    ...
                    onBeforeRender: function(data, node) {
                        ...
                        _.each(node.children, function(child, idx) {
                            ...
                            tabs += '<li data-idx="' + idx + '"' + (idx === 0 ? ' class="active"' : "") + '><a class="draggable tab" data-toggle="tab">' + escapeHTML(title) + "</a></li>";
                        });
                        data.tabs = tabs;
                    },
                    onInsert: function(evt, node) {
                ...
                selectfieldset: {
                    ...
                    onBeforeRender: function(data, node) {
                        ...
                        _.each(children, function(child, idx) {
                            tabs += '<option data-idx="' + idx + '" value="' + child.value + '"' + (child.node.active ? ' class="active"' : "") + ">" + escapeHTML(child.title) + "</option>";
                        });
                        tabs += "</select>";
                        data.tabs = tabs;
                        return data;
 ...
 */
            var getSchemaKey = function(schema, key) {
                var schemaKey = key.replace(/\./g, ".properties.").replace(/\[[0-9]*\](\.|$)/g, ".items$1");
 ...
 */
            var truncateToArrayDepth = function(key, arrayDepth) {
                ...
                pos = key.indexOf("[]", pos);
                if (pos === -1) return key; else return key.substring(0, pos);
            };
            /**
 ...
 */
            var getInitialValue = function(formObject, key, arrayPath, tpldata, usePreviousValues) {
                ...
                if (typeof value === "undefined") return null; else return value;
            };
            /**
            ...
            formNode.prototype.computeInitialValues = function(values, ignoreDefaultValues) {
                ...
                if (this.formElement) {
                    ...
                    if (this.formElement.id) {
                        ...
                        this.id = escapeSelector(this.ownerTree.formDesc.prefix) + "-elt-counter-" + _.uniqueId();
                    } else if (this.formElement.type === "button" || this.formElement.type === "selectfieldset" || this.formElement.type === "question" || this.formElement.type === "buttonquestion") {
            ...
            formNode.prototype.enhance = function() {
                ...
                if (this.formElement) {
                    ...
                    if (this.el) {
                        ...
                        if (this.onChange) $(this.el).bind("change", function(evt) {
                            node.onChange(evt, node);
                        });
                        if (this.view.onChange) $(this.el).bind("change", function(evt) {
                            node.view.onChange(evt, node);
                        });
                        if (this.formElement.onChange) $(this.el).bind("change", function(evt) {
                            node.formElement.onChange(evt, node);
                        });
                        if (this.onClick) $(this.el).bind("click", function(evt) {
                            node.onClick(evt, node);
                        });
                        if (this.view.onClick) $(this.el).bind("click", function(evt) {
                            node.view.onClick(evt, node);
                        });
                        if (this.formElement.onClick) $(this.el).bind("click", function(evt) {
                            node.formElement.onClick(evt, node);
                        });
                        if (this.onKeyUp) $(this.el).bind("keyup", function(evt) {
                            node.onKeyUp(evt, node);
                        });
                        if (this.view.onKeyUp) $(this.el).bind("keyup", function(evt) {
                            node.view.onKeyUp(evt, node);
                        });
                        if (this.formElement.onKeyUp) $(this.el).bind("keyup", function(evt) {
 ...
 */
            $.fn.jsonFormErrors = function(errors, options) {
                ...
                for (var i = 0; i < errors.length; i++) {
                    ...
                    var key = errors[i].uri.replace(/.*#\//, "").replace(/\//g, ".").replace(/\.([0-9]+)(\.|$)/, "[$1]$2");

 ...
 */
        ...
        (function($) {
            var methods = {
				...
				\*============================================================*/
                        var container = $('<div class="ax-container" style="position:relative;overflow:hidden;left:0;top:0;" />').appendTo($this.parent()).hover(function() {
                            if (settings.showControls) controlsDiv.show("fade");
                        }, function() {
                            if (settings.showControls) controlsDiv.hide("fade");
                        });
                        $this.data("container", container);
                        //mem for desotroy method
                        /*============================================================*\
				 controls div
				\*============================================================*/
                        var controlsDiv = $('<div class="ax-controls"/>').appendTo(container).hide().css({
                            "z-index": 1e3,
                            position: "absolute",
				...
				\*============================================================*/
                        if (settings.zoomIn) {
                            ...
                            if (zoomIn) zoomIn.bind(down, function(e) {
                                e.stopPropagation();
                                MOUSEX = MAIN_WIDTH / 2;
                        ...
                        if (settings.zoomOut) {
                            ...
                            if (zoomOut) zoomOut.bind(down, function(e) {
                                e.stopPropagation();
                                MOUSEX = MAIN_WIDTH / 2;
                                MOUSEY = MAIN_HEIGHT / 2;
                                $this.controlZoom(-.05);
                        ...
                        if (settings.reset) {
                            ...
                            if (reset) reset.bind(down, function(e) {
                                e.stopPropagation();
                                MOUSEX = MAIN_WIDTH / 2;
                                MOUSEY = MAIN_HEIGHT / 2;
                        ...
                        //sources
                        var MAIN_SRC = $this.attr("src");
                        //normal size source
                        var ZOOM_SRC = $this.attr("src-big");
                        //hight resolution source of image, to deprecheate 
                        if (settings.srcBig) ZOOM_SRC = settings.srcBig;
                        //hight resolution source of image
				...
				\*============================================================*/
                        ...
                        var START_X = 0;
                        //start X move position
                        var START_Y = 0;
                        //start Y move position
                        var MOUSEX = 0;
                        //mouse X relative to window DIV
                        var MOUSEY = 0;
                        //mouse Y relative to window DIV
                        var LIMIT_X = 0, LIMIT_Y = 0;
                        $this.ENABLE_DRAG = false;
                        //bind events to window div, not to image tag, we leave image tag free
                        container.bind(down, function(e) {
                            e.preventDefault();
                            ...
                            TOUCHNUM = coors[2];
                            if (e.shiftKey) $this.controlZoom(.05); else if (e.altKey) $this.controlZoom(-.05); else $this.ENABLE_DRAG = true;
                        ...
                        $this.drag = function() {
                            ...
                            //x:[LIMIT_X,0]		
                            if (new_x <= 0 && new_x >= LIMIT_X) $this.css({
                                left: new_x
                            });
                            //x:[LIMIT_Y,0]
                            if (new_y <= 0 && new_y >= LIMIT_Y) $this.css({
                                top: new_y
                            });
                        };
                        /******************************ZOOOM FUNCTIONS****************************/
                        ...
                        $this.zoomInOut = function(zoom) {
                            ...
                            if (new_w <= MAIN_WIDTH) new_w = MAIN_WIDTH;
                            //limit dimension			
                            ...
                            if (newzoom >> 0 <= settings.maxZoom) //a bit faster newzoom<<0 == Math.round(newzoom)
                            {
                                ...
                                //limits
                                if (new_x >= 0) new_x = 0;
                                //in this case the limit is 0 but for multiple 3d images may have a limit
                ...
                destroy: function() {
                    return this.each(function() {
                        ...
                        $this.css({
                            width: dims.width,
                            height: dims.height
                        }).appendTo(old.parent).attr("src", old.src).data("container").remove();
                        //remove container created by plugin
                        $(document).unbind(".axzoomer");
                    });
                },

define("#marked/0.2.6/marked-debug", [], function(require, exports, module) {
    (function() {
        /**
 * Block-Level Grammar
 */
        ...
        block.html = replace(block.html)("comment", /<!--[\s\S]*?-->/)("closed", /<(tag)[\s\S]+?<\/\1>/)("closing", /<tag(?:"[^"]*"|'[^']*'|[^'">])*?>/)(/tag/g, tag())();
 ...
 */
        ...
        block.token = function(src, tokens, top) {
            var src = src.replace(/^ +$/gm, ""), next, loose, cap, item, space, i, l;
            while (src) {
                // newline
 ...
 */
        inline.lexer = function(src) {
            var out = "", links = tokens.links, link, text, href, cap;
            while (src) {
                // escape
 ...
 */
        ...
        function tok() {
            switch (token.type) {
              ...
              case "list_start":
                {
                    var type = token.ordered ? "ol" : "ul", body = "";
 ...
 */
        ...
        function tag() {
            var tag = "(?!(?:" + "a|em|strong|small|s|cite|q|dfn|abbr|data|time|code" + "|var|samp|kbd|sub|sup|i|b|u|mark|ruby|rt|rp|bdi|bdo" + "|span|br|wbr|ins|del|img)\\b)\\w+(?!:/|@)\\b";
            return tag;
        }
